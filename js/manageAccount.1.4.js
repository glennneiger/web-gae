jQuery(document).ready(function(){	
			...
			jQuery('select.select_month').each(function(){
				var title = jQuery(this).attr('title');
				if( jQuery('option:selected', this).val() != ''  ) title = jQuery('option:selected',this).text();
				jQuery('select.select_month')
					.css({'z-index':10,'opacity':0,'-khtml-appearance':'none'})
					.after('<span class="select_month">' + title + '</span>')
					.change(function(){
			...
			jQuery('select.select_yr').each(function(){
				var title = jQuery(this).attr('title');
				if( jQuery('option:selected', this).val() != ''  ) title = jQuery('option:selected',this).text();
				jQuery('select.select_yr')
					.css({'z-index':10,'opacity':0,'-khtml-appearance':'none'})
					.after('<span class="select_yr">' + title + '</span>')
					.change(function(){
						val = jQuery('option:selected',this).text();
						jQuery(this).next().text(val);
						})
			});
			jQuery('select.select_country').each(function(){
				var title = jQuery(this).attr('title');
				if( jQuery('option:selected', this).val() != ''  ) title = jQuery('option:selected',this).text();
				jQuery('select.select_country')
					.css({'z-index':10,'opacity':0,'-khtml-appearance':'none'})
...
function submitUserInfo(){
    var manageEmail = jQuery('#manageemail').val();
    if(!isValidEmail(manageEmail,'manageemail'))
	{
	    return false;
	}
    var managePassword = jQuery('#managePassword').val();
    if(!(chkSpaceNull('managePassword',managePassword))){
    ...
    var confirmPassword = jQuery('#confirmPassword').val();
     ...
     if(confirmPassword!==managePassword){
	jQuery('#managePassword').addClass('input_fields_error');
	jQuery('#confirmPassword').addClass('input_fields_error');
	jQuery('#managePassword').focus();
    ...
    if(firstname==""){
	jQuery('#firstname').addClass('input_fields_error');
    ...
    if(lastname==""){
	jQuery('#lastname').addClass('input_fields_error');
	jQuery('#lastname').focus();
	return false;
     }
    
    var billingadd1 = jQuery('#billingadd1').val();
    if(billingadd1==""){
	jQuery('#billingadd1').addClass('input_fields_error');
	jQuery('#billingadd1').focus();
	return false;
     }
    
    var billingadd2 = jQuery('#billingadd2').val();
//    if(billingadd2==""){
//	jQuery('#billingadd2').addClass('input_fields_error');
//	jQuery('#billingadd2').focus();
//	return false;
//     }
    
    var city = jQuery('#city').val();
     if(city==""){
	jQuery('#city').addClass('input_fields_error');
	jQuery('#city').focus();
	return false;
     }
    ...
    var state = jQuery('#state').val();
     if(state==""){
	jQuery('#state').addClass('input_fields_error');
	jQuery('#state').focus();
	return false;
     }
    ...
    var zip = jQuery('#zip').val();
     if(zip==""){
	jQuery('#zip').addClass('input_fields_error');
	jQuery('#zip').focus();
	return false;
     }
    ...
    if(country==""){
	jQuery('#country').addClass('input_fields_error');
    ...
    if(phone==""){
	jQuery('#phone').addClass('input_fields_error');
	jQuery('#phone').focus();
	return false;
    }
    
    var manageAccountCreditType = jQuery('#manageAccountCreditType').val();
    if(manageAccountCreditType==""){
	jQuery('#manageAccountCreditType').addClass('input_fields_error');
	jQuery('#manageAccountCreditType').focus();
	return false;
    }
    
    var manageAccountCreditNumber = jQuery('#manageAccountCreditNumber').val();
    var ccSecurityCode = jQuery('#ccSecurityCode').val();
    var intRegex = /^\d+$/;
    if(intRegex.test(manageAccountCreditNumber)) {
    	if(ccSecurityCode==""){
			jQuery('#ccSecurityCode').addClass('input_fields_error');
			jQuery('#ccSecurityCode').focus();
			return false;
    	}
    	
    	if(manageAccountCreditNumber.indexOf('-')>=0){
        	jQuery('#manageAccountCreditNumber').addClass('input_fields_error');
        	jQuery('#manageAccountCreditNumber').focus();
        	return false;
        }
    }
    if(manageAccountCreditNumber==""){
	jQuery('#manageAccountCreditNumber').addClass('input_fields_error');
	jQuery('#manageAccountCreditNumber').focus();
	return false;
    }
    
    var manageAccountccYear = jQuery('#manageAccountccYear').val();
    if(manageAccountccYear==""){
		jQuery('#manageAccountccYear').addClass('input_fields_error');
		jQuery('#manageAccountccYear').focus();
		return false;
    }
    
    var manageAccountccMonth = jQuery('#manageAccountccMonth').val();
    if(manageAccountccMonth==""){
    	jQuery('#manageAccountccMonth').addClass('input_fields_error');
    	jQuery('#manageAccountccMonth').focus();
    	return false;
    }
                
    jQuery.ajax({
		type : "POST",
		url : host+"/subscription/register/manage_ajax.php",
		data : "type=manageAccount&manageEmail="+manageEmail+"&managePassword="+managePassword+"&firstname="+firstname+"&lastname="+lastname+"&billingadd1="+billingadd1+"&billingadd2="+billingadd2+"&city="+city+"&state="+state+"&zip="+zip+"&country="+country+"&phone="+phone+"&manageAccountCreditType="+manageAccountCreditType+"&manageAccountCreditNumber="+manageAccountCreditNumber+"&manageAccountccMonth="+manageAccountccMonth+"&manageAccountccYear="+manageAccountccYear+"&ccSecurityCode="+ccSecurityCode,
		error : function(){},
		success : function(res){
			...
			if(result.status==true){
				...
				jQuery("#messageDiv").html(result.msg);
			}else{
				jQuery("#messageDiv").empty();
			      jQuery("#errorMessageDiv").html(result.msg);	
			}
		}
	});
...
function isValidEmail(emails,divid)
{
	...
	{
		jQuery('#'+divid).addClass('input_fields_error');
		bools=false;
		return false;
	}
	return bools;
...
function checkPasswordField(fieldId)
{
	...
	//$(fieldId).value="";
	//jQuery('#'+fieldId).attr('type')='Password';
	jQuery('#'+fieldId).val('');
	

}

function chkSpaceNull(fieldId,str)
{
	var re = /^\s{1,}$/g; //match any white space including space, tab, form-feed, etc.
	if((str=='')||(str.length==0) || (str==null) || (str.search(re) > -1))
	{
	        jQuery('#'+fieldId).addClass('input_fields_error');
		return false;
	}
	else
	{
...
function removeErrorDiv(divid){
	if(jQuery('#'+divid).hasClass('input_fields_error')){
		jQuery('#'+divid).removeClass('input_fields_error')
	}
}
...
function getSelected(country,countryDiv){
	jQuery('select#'+countryDiv+' option[value="'+country+'"]').attr('selected', true);
}
