...
// needs to be optimized stockdata in related articles module
var stocksymbols = $j("#stock-tags").html();
if (stocksymbols != "" && stocksymbols != null) {
$j.post("/stocks/marketdata.php", { module: "stockquote", value: stocksymbols}, function(data){
var stockdata = eval(data);
stockdata = stockdata.toString();

stockdata = stockdata.replace(/,<style>/g, "</div><div style=\'clear:both;\'></div><style>");
stockdata = stockdata.replace(/<style>/g, "<div class=\'related-stock\'><style>");
$j("#related-stocks").html(stockdata + "</div><div style=\'clear:both;\'></div>");
});


$j("#txtTitle").bind("focus",function() { reloadstatus=0;});
$j("#txtBody").bind("focus",function() { reloadstatus=0;});
}

$j(".account_name img").bind("click", function() { closeControlPanel(); });
