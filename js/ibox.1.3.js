...
function init_ibox(linkFromTop,url) {
...
var ibox_type = 0;
	...
	if(urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif'){
		...
		imagePreloader.onload = function(){
			...
			imagePreloader = resizeImageToScreen(imagePreloader);
			hideIndicator();

			getElem('ibox_content').style.overflow = "hidden";

			var strHTML = "<a href=\"javascript:void(null);\"><img name=\"ibox_img\" src=\""+url+"\" style=\"width:"+imagePreloader.width+"px;height:"+imagePreloader.height+"px;border:0;\"/></a>";
					
			if(loadCancelled == false) {
				// set width and height
				ibox.style.height = imagePreloader.height+'px';
				ibox.style.width = imagePreloader.width+'px';
				ibox.style.display = "";
		...
		imagePreloader.src = url;
			...
			http.onreadystatechange = function() {
				var urlStatus=url.match("defaultadframe.php");
				if(http.readyState == 4){
					hideIndicator();
					
					/*alert("User defined height : "+params['height']);
					alert("User defined width : "+params['width']);*/
					 
					if(params['height']) {ibox.style.height = params['height']+'px';} 
					else {ibox.style.height = '400px';}
					if(params['width']) {ibox.style.width = params['width']+'px';} 
					else {ibox.style.width = '530px';}
					
					/*
					if(url.match('email_screen_2.htm'))
					{
						ibox.style.height = '490px';
					}
					else if(url.match('defaultadframe.php')){
						ibox.style.height = '600px';
					}
					else
					{
						ibox.style.height = '478px';
					}
					if(url.match('defaultadframe.php')){
						ibox.style.width = '955px';
					}
					else{
						ibox.style.width = '532px';
					}*/
					
					/*alert("Final height "+ibox.style.height);
					alert("Final width "+ibox.style.width);
					*/
					ibox.style.display = "";
					ibox.style.visibility = "hidden";
					posToCenter(ibox); 	
...
setIBoxContent = function(str) {
	...
	e.innerHTML = str;
	try{
		var r  = globaparam['targeturl'];
		 var rep = r.replace('~','?');
		  var rep1 = rep.replace('-','=');
		$('targeturl').value= rep1;
	   }
	catch(err)
        { 
            //alert(err);
...
//Function to close the ibox when clicked on close button of index.htm and register.htm.
function iboxclose(){
hideIbox();
}