<?php
	...
	global $D_R,$Podcast_video_file_path;
	...
	$filePath=$D_R."/".$Podcast_video_file_path;
...
/*	$file_path = $obAWSUpload->folder_name.$obAWSUpload->file_name;
	...
	if($output)
	{
		...
		$obRsync->uploadVideoAws("mvtv_podcast",$newFileName);
		echo "FILEID:" . $newFileName; // Return the file name to the script
		/*$obAWSUpload = new AWSUpload();
		$obAWSUpload->folder_name = $Podcast_video_file_path;
		...
		{
			echo 'Error in Podcast Video Upload to Amazon';
		}*/
	}
	else
	{
		echo 'Error in Podcast Video Upload';
	}
?>