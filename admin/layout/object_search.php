...
<table cellpadding="0" border="0" cellspacing="0" width="100%">
	<tr class="" id="search _header">
    <td width="50%" id="article_tab" onclick="toggleSearchTab('article')" class="article_tab_selected">Articles</td>
    <td width="50%" id="article_tab" onclick="toggleSearchTab('daily_feed')" class="article_tab">Daily Feed</td>
    <!--<td width="20%" id="video_tab" onclick="toggleSearchTab('video')" class="article_tab">Videos</td><td width="20%" id="buzzpost_tab" onclick="toggleSearchTab('buzzpost')" class="article_tab">BuzzPosts</td><td width="20%" id="discussion_tab" onclick="toggleSearchTab('discussion')" class="article_tab">Discussions</td><td width="20%" id="blog_tab" onclick="toggleSearchTab('blog')" class="article_tab">Blogs</td>-->
    </tr>
	<tr><td  colspan="2"><table id="search_result_table">
	<tr id="searched_article">
		<td>
			...
			?>			
			<a href='#' onclick="selectSerchedItems('<?= addslashes($articleTitle)?>','<?= $articleRow['item_id'].":".$articleRow['item_type'] ?>')"><?= $articleRow['title']; ?> by <?= $articleRow['author'];?></a> - <span class="date"><?= date('F d, Y g:i a',$articleRow['date']); ?></span><br>
			<? } 
			}
			else
			{
    ...
    <tr id="searched_daily_feed" style="display:none;">
		<td>
			<div style="overflow:scroll;width:400px;height:120px;border:1px solid black">
			<? 
			if(is_array($arSearchResult[18]))
			{
				foreach($arSearchResult[18] as  $articleRow)
				{ 
				//$articleTitle = str_replace('"', '&quot;', $articleRow['title']); 				
				$articleTitle = htmlentities($articleRow['title']);
