...
$cnt       = 0;
 if($method=='dellogo')
 {
    $query   = "select id  from  buzzbanter where  branded_img_id='$module_id';";
    @$result1  =  exec_query($query);
   if(count($result1)>0)
   {
    $Branded_Article = get_Branded_Buzz($module_id);
	$string ="<div><table width='100%' border='0' cellpadding='1' cellspacing='1'>
	          <tr ><td valign='middle' align='left'  colspan='2'><font color='#FF0000'><b>First Delete Buzz Branded Logo
			   From The Following Buzz Post(s)</b></font></td>
			  <td valign='middle' align='right'><a onclick='displayRow()'>Hide Buzz Post(s)</a></td>
              </tr>";
  ...
  $j=1;
   ...
   $string .="<tr class='highlight_row'><td width='15%' valign='middle' align='left'>$j</td>
			  ...
			  <a  href='".$HTPFX.$HTHOST."/admin/banter.htm?contributor_id=".$module['contrib_id'] ."&id=".$module['id']. "'
			   TARGET='_blank'>
			   edit</a></td></tr>";
    ...
    {
           $query1   = "DELETE FROM buzz_branded_images WHERE id='$module_id'";
...
<tr>
  ...
  <tr>
   <td width="15%" valign="middle" class="admin_module_head" align="left">S.NO</td>
    <td width="58%" valign="middle" class="admin_module_head" align="left">Buzz Branded Logo</td>
  ...
  <td width="15%" valign="middle" align="left"><?=$i ?></td>
    <td width="58%" valign="middle" ><?= trim(htmlentities($module['name'])) ?></td>
	 ...
	 <a onClick="buzzbrandedlogo_confirm('<?= $module['id'] ?>','listbuzzbrandedlogo.php','dellogo')" >delete</a></td>
  </tr>
