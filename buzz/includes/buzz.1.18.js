...
function checkLatestTimeStamp (req) {
	...
	if ((currentLatest > 0) && (currentLatest > latestTimeStamp)) {
		...
		var pars = 'sid=' + sid+'&time='+milliseconds;
   ...
   if (req.responseText.length > 0) {
	...
	if (post.ID != latestID || (nav.noPrev == true && nav.lastPage == 1)) {
		...
		if (post.type=="ARTICLE")
			{postLink = 'javascript:launchPage("' + post.url + '","articleLink",0);'} // if an article then the whole function call is built into the postID otherwise it's just a number
		if (post.type=="DAILYFEED")
			{postLink = 'javascript:launchPage("' + post.url + '","dailyfeedlink",0);'} 	
		else
		...
		//alert("go for the alert");
			...
			if (post.type=="POST")
				...
				alertText += 'function letsGo() {opener.launchPage("' + post.url +'","articleLink",0);self.close();}';
			else if (post.type=="DAILYFEED")
				alertText += 'function letsGo() {opener.launchPage("' + post.url + '","dailyfeedLink",0);self.close();}';
...
function submitSearchForm() {
	...
	var pars = 'q=' + $F('q') + '&mo=' + mo + '&day=' + day + '&year=' + year + '&author=' + encodeURIComponent($F('author'));
	var myAjax = new Ajax.Request(url, {method: 'post', parameters: pars, onLoading: showProgress('shwprogressdataval'), onComplete:finishSearchRequest});
	return false;
}
...
function showProgress (id) { // puts spinner in specified div
	var x = $(id); 	
	x.innerHTML = '<span style="color:#565959;font-size:11px;padding-left:3px;padding-bottom:1px;  background-color:#ffffcc;position:absolute;z-index:1000;">Loading....</span>';
		
	//'<div class="progress">Loading... <img src="images/spinner.gif" /></div>';
...
function loadPage(page) {
	...
	var myAjax2 = new Ajax.Request(url, {method: 'post', parameters: pars, onLoading:showProgress('shwprogressdata'), onComplete:finishLoadingPage});
...
//	if (req.responseText.length>0) {
	...
	var reval =  req.responseText;
	 if(reval.search(/<div id="/)!=-1){
			$('articles').innerHTML = req.responseText;
		}else if(reval!=='') {
			var post = eval('('+req.responseText+')');//alert(post.status);	
			var host=window.location.protocol+"//"+window.location.host;//alert(host);
			if(post.status=='redirect'){
				window.location.href=host+'/gazette/buzzbanter/signin.htm?err=1';				 
			}		
		}
	$('shwprogressdata').innerHTML = '';
...
function jumpToLatestPost() {
	...
	var pars = 'sid=' + sid + '&latest=1&chars=' + characters + "&posts=" + posts_per_page+'&time='+milliseconds;
	jumpToId = 'latest';
...
function jumpRequest(pars) {
	var url = '_content.php';
	var myAjax4 = new Ajax.Request(url, {method: 'post', parameters: pars, onLoading:showProgress('shwprogressdata'), onComplete:finishJump});
}
...
function finishJump(req) {
    ...
    //if (req.responseText.indexOf("There are no posts to display") == -1) {
		...
		$('shwprogressdata').innerHTML = '';
		 var reval = req.responseText;//alert(reval);
		 var host=window.location.protocol+"//"+window.location.host;//alert(host);	
        if(reval.search(/<div id="/)!=-1){
			$('articles').innerHTML = req.responseText;
		}else if(reval!=='') {
...
function makeLinksIntoPopups() {		
    ...
    for (var j=0; j<popup2.length; j++) {    	
	    ...
	    for (var i=0; i<links.length-2; i++) {
	      ...
	      if(url.indexOf("mailto") == 0) // Implement not to make links on mailto tag
		  {
			  continue;
		  }
		  if (url.indexOf("bookmark.php") == -1) {
	        args = "javascript:launchPage('" + addslashes(url) + "','" + name + "',0);void(0);";
	  	  } else {
	  	  	args = "javascript:launchBookmarkFromURL('" + addslashes(url) + "');void(0);";
	  	  }
        links[i].href = args;
	  	}
	}
...
function init() { 
	...
	***********************************************/
	...
	Event.observe('mo','mouseover',enableclick,true);
	Event.observe('mo','mouseout',disableclick,true);
	Event.observe('day','mouseover',enableclick,true);
	Event.observe('day','mouseout',disableclick,true);
	Event.observe('year','mouseover',enableclick,true);
	Event.observe('year','mouseout',disableclick,true);
	Event.observe('author','mouseover',enableclick,true);
	Event.observe('author','mouseout',disableclick,true);
