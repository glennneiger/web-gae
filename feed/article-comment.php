...
foreach($list as $key){
	...
	$content = html_entity_decode($key['content'],ENT_QUOTES);
	//$title	=	mswordReplaceSpecialChars(strip_tags($key['title']));
	$title = html_entity_decode($key['title'],ENT_QUOTES);
	...
	<item>
		...
		<!-- creation date of thread (article) -->
     	 <wp:post_date_gmt><?=date("Y-m-d H:i:s",strtotime($key['publish_date']));?></wp:post_date_gmt>
		 	...
		 	?>
			  <wp:comment>
					...
					<!-- parent id (match up with wp:comment_id) -->
					 <wp:comment_parent><?=$key['threadId']?></wp:comment_parent>
			  </wp:comment>
		 <?php } ?>
	</item>
<?php } ?>
