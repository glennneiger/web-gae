<?
	...
	function get_full_ss_alerts($alert_id, $approvedonly=1){
		...
		$qry="select a.*, UNIX_TIMESTAMP(a.date)udate from ss_alerts a where a.type = 'alert' and a.id='$alert_id'";
		if($approvedonly)
	...
	function get_full_qp_blogs($alert_id, $approvedonly=1){
		...
		if($_GET[AMADMIN])$approvedonly=0;
		if(!$alert_id)return array();
		$qry="select a.*, UNIX_TIMESTAMP(a.date)udate from ss_alerts a where a.type = 'blogs' and a.id='$alert_id'";
		if($approvedonly)
			$qry.=" AND a.approved='1' ";
		$ret=exec_query($qry,1);
		if(count($ret)){
    ...
    function get_full_qp_video($video_id, $approvedonly=1){
		...
		$qry="SELECT * from ss_mvtv where id='$video_id'";
		if($approvedonly)
			$qry.=" AND a.approved='1'";
		$ret=exec_query($qry,1);
