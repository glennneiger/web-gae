...
class Recurly_Client
{
  ...
  */
  private function _sendRequest($method, $uri, $data = '')
  {
    ...
    $result = file_get_contents($uri,false,$contextApi);

 $status = explode('HTTP/1.1 ',$http_response_header[0]);
    ...
    $headers = $this->_getHeaders($http_response_header);
  
  return new Recurly_ClientResponse($status[1], $headers,  $result);
