...
class AmazonSNS
{
	...
	private $protocol = 'http://'; // http is allowed
	...
	private function _request($action, $params = array())
	{
		try {
			...
			// Check return code
			if($this->_checkGoodResponse($info['0']) === false)
			{
				// Response not in 200 range
				if(isset($xmlResponse->Error))
				{
					// Amazon returned an XML error
					throw new SNSException(strval($xmlResponse->Error->Code) . ': ' . strval($xmlResponse->Error->Message), $info['0']);
				}
				else
				{
					// Some other problem
					throw new APIException('There was a problem executing this request',$info['0']);
				}
			}
			else
			{
