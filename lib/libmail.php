...
*/
function BuildMail($showsubject=0)
{

	// build the headers
	$this->headers = "";
	//$this->xheaders['To'] = implode( ", ", array_unique($this->sendto) );
	
	if( count($this->acc) > 0 )
		$this->xheaders['CC'] = implode( ", ", array_unique($this->acc) );
	
	if( count($this->abcc) > 0 ) 
		$this->xheaders['Bcc'] = implode( ", ", array_unique($this->abcc) );
	...
	if( $this->charset != "" ) {
		...
		$this->xheaders["Content-Type"] = "$this->defaultType; charset=$this->charset";
...
function Send()
{
	...
	$this->strTo = implode( ", ", array_unique($this->sendto ));
...
function ValidEmail($address)
{
	...
	if(preg_match("/[\w\-\.]{2,255}@.{2,255}\.\w{2,255}/",$address))
 		return true;
 	else
 		return false;
}
...
function _build_attachement()
{
	...
	$this->fullBody .= "Content-Type: $this->defaultType; charset=$this->charset\nContent-Transfer-Encoding: $this->ctencoding\n\n" . $this->body ."\n";
